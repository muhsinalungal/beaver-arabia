...
class SummaryWork(models.Model):
    ...
    sale_project_id = fields.Many2one('sale.project',string='Sale Project Ref.')
...
class Projects(models.Model):
    ...
    estimate_detail_ids = fields.One2many('sale.estimate.details','project_id','Estimate Detail')
    ...
    estimate_count = fields.Integer(string='Estimate Count', compute='get_estimate_count', readonly=True)
    estimate_line_count = fields.Integer(string='Estimate Line Count', compute='get_estimate_line_count', readonly=True)
    estimate_detail_count = fields.Integer(string='Estimate Detail Count', compute='get_estimate_detail_count', readonly=True)
    ...
    microtunneling_setup = fields.Float(string="Microtunneling Setup", size=8)
    ...
    @api.depends('total_wages','total_equipment','total_general','total_material')
    def _compute_total_cost(self):
        for rec in self:
            total_amount = rec.total_wages + rec.total_equipment + rec.total_general + rec.total_material
            contigency = total_amount * 0.1
    ...
    # @api.depends('total_wages','wages_markup','estimate_line_ids.subtotal')
    # def _compute_wages_sale(self):
    #     if self.total_wages and self.wages_markup:
    #         perc = 1+(self.wages_markup/100)
    #         self.wages_sale = self.total_wages*perc
    ...
    #             total_amount += line.subtotal
    #         rec.write({'total_amount': total_amount})
    ...
    def get_estimate_detail_count(self):
        for rec in self:
            estimate_detail = self.env['sale.estimate.details'].search([('project_id', "=", rec.id)])
            rec.write({'estimate_detail_count': len(estimate_detail)})
...
#         return res
    ...
    def action_view_estimate_details(self):
        estimates = self.env['sale.estimate.details'].search([('project_id', "=", self.id)])
        action = self.env['ir.actions.act_window']._for_xml_id('bt_job_cost_estimation.action_estimate_detail')
        action['context'] = {'default_project_id': self.id, 'group_by': 'estimate_id'}
