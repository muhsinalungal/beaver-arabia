...
class HrEmployee(models.Model):
    ...
    division_id = fields.Many2one('journal.code', string="Division")
    state = fields.Selection([('draft', 'Draft'),
                             ('valid', 'Valid')
                             ],string="State", default='draft')
    no_sponsorship = fields.Char(string="No. Sponsorship Transfer")
    vac_days = fields.Char(string="Vac. Days")
    vac_cy = fields.Char(string="Vac. Cy")
    vehcle = fields.Selection([('yes', 'Yes'),('no', 'No')], string="Vehicle")
    sim_card = fields.Selection([('yes', 'Yes'),('no', 'No')], string="Sim Card")
    street = fields.Char()
    street2 = fields.Char()
    ...
    divisioned_id = fields.Many2one('journal.code', string="Division")
    insurance_company_id = fields.Many2one('insurance.company', string="Insurance Company")
    insurance_exp_date = fields.Date(string="Insurance Expiry")
    archive = fields.Boolean(default=False, help="Set active to True to hide the Employee without removing it.")
    
    def click_to_valid(self):
