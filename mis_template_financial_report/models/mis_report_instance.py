...
class MisReportInstance(models.Model):
    ...
    head_name = fields.Selection([
        ...
        ('comparitive', 'Income Statement - Comparative'),
    ],string='Name')

    project_ids = fields.Many2many(
        comodel_name="account.analytic.account", string="Filter Project"
    )
    is_balance_sheet = fields.Boolean(string="Is Balance Sheet",related="report_id.is_balance_sheet",
        store=True
    )
    is_project = fields.Boolean(string="Is Project",related="report_id.is_project",
        store=True
    )
    project_codes = fields.Char(string="Project Codes",compute="_compute_project_det",store=True)
    customer_name = fields.Char(string="Customer",compute="_compute_project_det",store=True)
    project_name = fields.Char(string="Project",compute="_compute_project_det",store=True)
    period_month = fields.Char(string="Period",compute="_compute_project_det",store=True)
    period_year = fields.Char(string="Year",compute="_compute_project_det",store=True)
    
    cost_center_ids = fields.Many2many(
        comodel_name="account.budget.post", string="Filter Cost Center"
    )
    date_all = fields.Boolean(string="All",
        store=True
    )
    partner_ids = fields.Many2many(
        comodel_name="res.partner", string="Filter Customer"
    )


    @api.depends("date_from","date_to","is_project","report_id","project_ids")
    def _compute_project_det(self):
        for instance in self:
            if instance.is_project == True and instance.project_ids:
                if len(instance.project_ids) == 1:
                    instance.project_codes = instance.project_ids[0].name
                    instance.project_name = instance.project_ids[0].name
                    instance.customer_name = instance.project_ids[0].partner_id.name
                else:
                    project_codes = ""
                    for project in instance.project_ids:
                        project_codes += '/' + project.name
                    instance.project_codes = project_codes
                    instance.project_name = ''
                    instance.customer_name = ''

                st_date = instance.date_from
                end_date =instance.date_to
                instance.period_month = st_date.strftime('%b')[:3]+ ' ' + 'To' + ' '+end_date.strftime('%b')[:3]
                instance.period_year = end_date.strftime('%Y')
    ...
    @api.depends("date_from","month_wise","dept_wise","compare_year")
    def _compute_comparison_mode(self):
        for instance in self:
            if instance.compare_year or instance.month_wise or instance.dept_wise or instance.project_ids:
    ...
    def _compute_note(self):
        for instance in self:
            if not instance.is_balance_sheet:
                    ...
                    if instance.date_to:
                        ...
                        if st_date and end_date:
                            ...
                            if months == 0:
                                ...
                                instance.note = "For The Month from %s to %s %s" % (st_date.strftime('%b'),end_date.strftime('%b'),end_date.strftime('%Y'))
            if instance.is_balance_sheet:
                if instance.date_to and not instance.compare_year:
                        st_date = instance.date_from
                        end_date =instance.date_to
                        if st_date and end_date:
                            months = (end_date.year - st_date.year) * 12 + (end_date.month - st_date.month)
                            if months == 0:
                                ...
                                last_date = datetime.datetime(int(year), 12, 31)
                                if last_date.date()==instance.date_to:
                                    instance.note = "AS AT %s 31, %s" % (last_date.strftime('%B'),last_date.strftime('%Y'))

                            else:
                                instance.note = "For The Month from %s to %s %s" % (st_date.strftime('%b'),end_date.strftime('%b'),end_date.strftime('%Y'))
                if instance.date_to and instance.compare_year:
                        st_date = instance.date_from
                        end_date =instance.date_to
                        if st_date and end_date:
                            months = (end_date.year - st_date.year) * 12 + (end_date.month - st_date.month)
                            if months == 0:
                                ...
                                last_date = datetime.datetime(int(year), 12, 31)
                                if last_date.date()==instance.date_to:
                                    instance.note = "AS AT %s 31, %s & %s" % (last_date.strftime('%B'),last_date.strftime('%Y'),instance.compare_year)
    ...
    def _inverse_comparison_mode(self):
        for record in self:
            if not record.comparison_mode:
                ...
                record.write({"period_ids": [(0, 0, {"name": "Amount"})]})
            # else:
    ...
    @api.onchange("quarter_wise","year","date_all")
        ...
        if self.dept_wise == True:
            ...
            for project in self.project_ids:
                vals = {
                    ...
                    'dept_ids' : tuple(self.dept_ids.ids),
                    'cost_center_ids' : tuple(self.cost_center_ids.ids),
