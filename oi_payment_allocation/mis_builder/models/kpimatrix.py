...
class KpiMatrixCell(object):  # noqa: B903 (immutable data class)
    def __init__(
        ...
        self.row = row
        self.subcol = subcol
        if self.row.label in ('Revenue','Cost Of Sales','General & Administrative Expenses','Other Income(Expense)'):
            self.val = ''
        elif self.row.label in ('Total Revenue','Total Cost of Sales','Total G&A Expenses','Other Income(Expense) Total','Net Income','Gross Profit','Operating Income & Loss'):
        ...
        for row in self.iter_rows():
            ...
            for cell in row.iter_cells():
                if cell is None:
                    ...
                    if not row.row_id in ('revenue','cost_of_sales','general_administrative_expenses','other_income_exp'):
