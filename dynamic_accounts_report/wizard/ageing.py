...
class AgeingView(models.TransientModel):
    ...
    account_ids = fields.Many2many(
        'account.account', string='Partner'
    )
    invoice_type_ids = fields.Many2many(
        'invoice.type', string='Inv.Type'
    )
    ...
    def get_filter(self, option):
        ...
        if data.get('journal_code'):
            filters['journal_code'] = self.env['journal.code'].browse(
                data.get('journal_code')).mapped('name')
        else:
            filters['journal_code'] = ['All']
        if data.get('invoice_type'):
            filters['invoice_type'] = self.env['invoice.type'].browse(
                data.get('invoice_type')).mapped('name')
        else:
            filters['invoice_type'] = ['All']

        if data.get('accounts'):
            filters['accounts'] = self.env['account.account'].browse(
                data.get('accounts')).mapped('name')
        else:
    ...
    def get_filter_data(self, option):
        ...
        accounts = r.account_ids if r.account_ids \
            else self.env['account.account'].search([])
        journal_code = r.journal_code_ids if r.journal_code_ids \
            else self.env['journal.code'].search([])
        invoice_type = r.invoice_type_ids if r.invoice_type_ids \
            else self.env['invoice.type'].search([])

        filter_dict = {
            'partners': r.partner_ids.ids,
            'accounts': r.account_ids.ids,
    ...
    def _get_report_values(self, data):
        ...
        if data['result_selection'] == 'customer':
            account_type = ['receivable','other']
        elif data['result_selection'] == 'supplier':
            account_type = ['payable','other']
        else:
            account_type = ['payable', 'receivable','other']
        target_move = data['target_move']
        partners = data.get('partners')
        accounts = data.get('accounts')
        journal_code = data.get('journal_code')
        invoice_type = data.get('invoice_type')
        result_selection = data.get('result_selection')
        ...
        account_res = self._get_partner_move_lines(data, partners, date_from,
                                                   target_move,
                                                   account_type,accounts,result_selection,
                                                   data['period_length'],journal_code,invoice_type)
    ...
    def write(self, vals):
        ...
        if vals.get('journal_code_ids'):
            vals.update(
                {'journal_code_ids': [(4, j) for j in vals.get('journal_code_ids')]})
        if not vals.get('journal_code_ids'):
            vals.update({'journal_code_ids': [(5,)]})
        if vals.get('invoice_type_ids'):
            vals.update(
                {'invoice_type_ids': [(4, j) for j in vals.get('invoice_type_ids')]})
    ...
    def _get_partner_move_lines(self, data, partners, date_from, target_move,
                                account_type,accounts,result_selection,
                                period_length,journal_code,invoice_type):
        ...
        account_list = '(l.account_id IS NOT  NULL)'
        acc_list = tuple(accounts.ids) + tuple([0])
        if accounts:
            
            if acc_list:
                account_list = '(l.account_id IN %s)'
                arg_list += (tuple(acc_list),)
        ...
        if result_selection == 'customer':
            ...
            invoice_type_ids = []
            if journal_code:                
                journal_code_ids =journal_code.ids
            if invoice_type:               
                invoice_type_ids = invoice_type.ids
            if not invoice_type:                
                invoice_types = self.env['invoice.type'].search([])
                invoice_type_ids = invoice_types.ids
                
            if not journal_code:
                journal_codes = self.env['journal.code'].search([])
                journal_code_ids = journal_codes.ids
            ...
            if accounts:
                query = '''SELECT l.id,l.date
                                ...
                                WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
                                    ...
                                    AND ((am.journal_code_id IN %s))
                                    AND ((am.invoice_type_id IN %s))
                                AND (l.date <= %s)
                                AND l.company_id IN %s
                                AND l.account_id IN %s''' '''ORDER BY l.date asc'''
                cr.execute(query, (
                    tuple(move_state), tuple(account_type), date_from,
                    tuple(partner_ids),tuple(journal_code_ids), tuple(invoice_type_ids),date_from, tuple(company_ids),tuple(acc_list)))
            else:
                query = '''SELECT l.id,l.date
                            FROM account_move_line AS l, account_account, account_move am
                            WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
                ...
                cr.execute(query, (
                    tuple(move_state), tuple(account_type), date_from,
                    tuple(partner_ids),tuple(journal_code_ids), tuple(invoice_type_ids), date_from, tuple(company_ids),))
            aml_ids = cr.fetchall()
            ...
            for line in self.env['account.move.line'].browse(aml_ids):
                ...
                if partner_id:
                    ...
                    date_maturity = line.date_maturity or line.move_id.date
            ...
            for i in range(5):
                ...
                if accounts:
                    ...
                    args_list += (date_from, tuple(company_ids),tuple(acc_list),tuple(journal_code_ids),tuple(invoice_type_ids))
                    ...
                    query = '''SELECT l.id,l.date
                                    ...
                                    AND l.account_id IN %s
                                    AND ((am.journal_code_id IN %s))
                                    AND ((am.invoice_type_id IN %s))''' '''ORDER BY l.date asc'''
                    cr.execute(query, args_list)
                else:
                    args_list += (date_from, tuple(company_ids))
                    if journal_code_ids and invoice_type_ids:
                        args_list += (tuple(journal_code_ids),tuple(invoice_type_ids))
            ...
            if accounts:
                query = '''SELECT l.id,l.date
                                ...
                                WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
                                    ...
                                    AND (COALESCE(l.date_maturity,l.date) >= %s)\
                                    AND ((l.partner_id IN %s) OR (l.partner_id IS NULL))
                                    AND ((am.journal_code_id IN %s) OR (am.journal_code_id IS NULL))
                                    AND ((am.invoice_type_id IN %s) OR (am.invoice_type_id IS NULL))
                                AND (l.date <= %s)
                                AND l.company_id IN %s
                                AND l.account_id IN %s''''''ORDER BY l.date asc'''
                cr.execute(query, (
            ...
            for i in range(5):
                ...
                if accounts:
                    ...
                    args_list += (date_from, tuple(company_ids),tuple(journal_code_ids),tuple(invoice_type_ids))
