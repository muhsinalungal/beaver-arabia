odoo.define('dynamic_accounts_report.ageing', function (require) {
    ...
    template: 'AgeingTemp',
                ...
                try{
                    ...
                    _.each(datas['report_lines'][0], function(rep_lines) {
                            ...
                            if (initial_render) {
                                    ...
                                    self.$el.find('.account_selection').select2({
                                        placeholder: ' Accounts...',
                                    });
                                    self.$el.find('.journal_code').select2({
                                        placeholder: ' Journal Code...',
                                    });
                                    self.$el.find('.invoice_type').select2({
        ...
        apply_filter: function(event) {
            ...
            var account_ids = [];
            var account_text = [];
            var span_res = document.getElementById("accounts_res")
            var accounts_list = $(".account_selection").select2('data')
            for (var i = 0; i < accounts_list.length; i++) {
            if(accounts_list[i].element[0].selected === true)
            {account_ids.push(parseInt(accounts_list[i].id))
            if(account_text.includes(accounts_list[i].text) === false)
            {account_text.push(accounts_list[i].text)
            }
            ...
            var span_res = document.getElementById("journal_code_res")
            ...
            span_res.value = journal_code_text
            span_res.innerHTML=span_res.value;
            }
            }
            ...
            var invoice_type_ids = [];
            var invoice_type_text = [];
            var span_res = document.getElementById("invoice_type_res")
            var invoice_type_list = $(".invoice_type").select2('data')
            for (var i = 0; i < invoice_type_list.length; i++) {
            if(invoice_type_list[i].element[0].selected === true)
            {invoice_type_ids.push(parseInt(invoice_type_list[i].id))
            if(invoice_type_text.includes(invoice_type_list[i].text) === false)
            {invoice_type_text.push(invoice_type_list[i].text)
            }
            span_res.value = invoice_type_text
            span_res.innerHTML=span_res.value;
            }
            }
            if (invoice_type_list.length == 0){
            span_res.value = ""
            span_res.innerHTML="";
            }
            filter_data_selected.invoice_type_ids = invoice_type_ids
