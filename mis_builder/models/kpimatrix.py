...
class KpiMatrixRow(object):
    ...
    def account_code(self):
        if not self.account_id:
            ...
            return self._matrix.get_account_code(self.account_id)
...
class KpiMatrixCell(object):  # noqa: B903 (immutable data class)
    def __init__(
        ...
        self.row = row
        self.subcol = subcol
        if self.row.label in ('Revenue','Cost Of Sales','General & Administrative Expenses','Other Income(Expense)','Assets','Current Assets','Cash and Cash Equivalents',
                                'Prepayments & Other Receivables','Non-Current Assets','Property & Equipment','Other Non-Current Assets','LIABILITIES AND OWNERS EQUITY','Current Liabilities',
                                'Accrued & Other Payables','Non-Current Liabilities','Owners Equity','-',
                                'Material.','Equipment.','Equipment/Vehicle Own.','Subcontract.','Transportation.','Labour.','Labour Own.','Vehicle Hire & Other Exp.','Third Party Inspection(Veh/Equip/Man).','Fuel Expenses.','Vehicle Runn & Maint.Exp.','Repair & Maint.Equipments.','Other Costs.'):
            self.val = ''
        elif self.row.label in ('Total Revenue','Total Cost of Sales','Total G&A Expenses','Other Income(Expense) Total','Net Income','Gross Profit','Operating Income & Loss','Total Current Assets',
                                'Total Non-Current Assets','Total Non-Current Assets','TOTAL ASSETS','Total Current Liabilities','Total Non-Current Liabilities','Total Owners Equity','TOTAL LIABILITIES AND OWNERS EQUITY'):
            self.val = val or '0.00'
        else:
            self.val = val
        if self.row.label in ('Revenue','Cost Of Sales','General & Administrative Expenses','Other Income(Expense)','Assets','Current Assets','Cash and Cash Equivalents',
                                'Prepayments & Other Receivables','Non-Current Assets','Property & Equipment','Other Non-Current Assets','LIABILITIES AND OWNERS EQUITY','Current Liabilities',
            ...
            self.val_rendered = ''
                                ...
                                'Total Non-Current Assets','Total Non-Current Assets','TOTAL ASSETS','Total Current Liabilities','Total Non-Current Liabilities','Total Owners Equity','TOTAL LIABILITIES AND OWNERS EQUITY'):
            self.val_rendered = val_rendered or '0.00'
        else:
...
class KpiMatrix(object):
    ...
    def iter_rows(self):
        ...
        for kpi_row in self._kpi_rows.values():
            ...
            detail_rows = sorted(detail_rows, key=lambda r: r.account_code)
    ...
    def _load_account_names(self):
        ...
        accounts = self._account_model.search([("id", "in", list(account_ids))],order='code asc')
        self._account_names = {a.id: self._get_account_name(a) for a in accounts}

    def _load_account_codes(self):
        account_ids = set()
        for detail_rows in self._detail_rows.values():
            account_ids.update(detail_rows.keys())
        accounts = self._account_model.search([("id", "in", list(account_ids))],order='code asc')
        self._account_codes = {a.id: self._get_account_code(a) for a in accounts}

    def _get_account_name(self, account):
        result = u"{}".format(account.name)
        if self._multi_company:
            result = u"{} [{}]".format(result, account.company_id.name)
        return result

    def _get_account_code(self, account):
        result = u"{}".format(account.code)
        if self._multi_company:
            result = u"{} [{}]".format(result, account.company_id.name)
        return result
    ...
    def get_account_code(self, account_id):
        if account_id not in self._account_codes:
            self._load_account_codes()
        return self._account_codes[account_id] 
            ...
            for cell in row.iter_cells():
                if cell is None:
                    ...
                    if cell.val is AccountingNone or isinstance(cell.val, DataError):
                        val = None
                    elif row.row_id in ('total_revenue','total_cost_of_sales','total_g_a_expenses','other_income_expense_total','operating_income_loss','gross_profit','net_income',
                                        'total_current_assets','total_non_current_asset','total_assets','total_current_liabilities','total_non_current_liabilities',
                                        'total_owners_equity','total_liabilities_and_owners_equity'):
                        val = cell.val or '0.00'
                    else:
                        val = cell.val
                    col_data = {
                        "val": val,
                    ...
                    if not row.row_id in ('revenue','cost_of_sales','general_administrative_expenses','other_income_exp','assets','current_assets','cash_and_cash_equivalents','Prepayments & Other Receivables',
                                            'Non-Current Assets','property','other_non_current_assets','liabilities_and_owners_equity','current_liabilities','accrued_other_payables','non_current_liabilities',
                                            'owners_equity','cl_rest','ca_rest','material_total','equipment_total','equipment_vehicle_own_','subcontract_','transportation_','labour_total_','labour_own_total','vehicle_hire_other_exp_','third_party_inspection_veh_equip_man_'
                                            'fuel_expenses_','vehicle_runn_maint_exp_','repair_maint_equipments_','other_costs_'):
