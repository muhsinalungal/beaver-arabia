...
class MisReport(models.Model):
    ...
    * a list of KPI to be evaluated based on the variables resulting
    ...
    is_balance_sheet = fields.Boolean(
        string="Is Balance Sheet"
    )
    is_project = fields.Boolean(
        string="Is Project Report"
    )
    ...
    def _declare_and_compute_period(
        ...
        for subreport in self.subreport_ids:
            if subreport.name == 'subreport_pl_init':
                expression_evaluator_date_from=expression_evaluator.date_from
                expression_evaluator_date_to=expression_evaluator.date_to
                first_date = datetime.datetime(int(expression_evaluator.date_from.year-1), 1, 1)
                end_date = datetime.datetime(int(expression_evaluator.date_from.year-1), 12, 31)
                expression_evaluator.date_from = first_date
                expression_evaluator.date_to = end_date
                ...
                expression_evaluator.date_from = expression_evaluator_date_from
                expression_evaluator.date_to = expression_evaluator_date_to
            elif subreport.name == 'subreport_pl':
                expression_evaluator_date_from=expression_evaluator.date_from
                expression_evaluator_date_to=expression_evaluator.date_to
                first_date = datetime.datetime(int(expression_evaluator.date_from.year), 1, 1)
                end_date = datetime.datetime(int(expression_evaluator.date_from.year-1), 12, 31)
                expression_evaluator.date_from = first_date
                expression_evaluator.date_to = expression_evaluator.date_to
                subreport_locals_dict = subreport.subreport_id._evaluate(
                    expression_evaluator, subkpis_filter, get_additional_query_filter
                )
                locals_dict[subreport.name] = AutoStruct(
