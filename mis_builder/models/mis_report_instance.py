...
class MisReportInstancePeriod(models.Model):
    ...
    department_id = fields.Many2one(
        ...
        string="Department",
        help=(
            "Filter column on journal entries that match this Department."
            "This filter is combined with a AND with the report-level filters "
            "and cannot be modified in the preview."
        ),
    )
    ...
    analytic_account_ids = fields.Many2many(
        comodel_name="account.analytic.account", string="Filter Projects"
    )
    ...
    def _get_additional_move_line_filter(self):
        ...
        if self.department_id:
            domain.append(("cost_center_id", "=", self.department_id.id))
        if self.dept_ids:
            domain.append(("cost_center_id", "in", self.dept_ids.ids))
        if self.cost_center_ids:
            domain.append(("budgetry_position_id", "in", self.cost_center_ids.ids))
        if self.partner_ids:
...
class MisReportInstance(models.Model):
    ...
    def compute(self):
        ...
        matrix = kpi_matrix.as_dict()
        matrix.update({"is_balance_sheet":self.is_balance_sheet})
        return matrix
