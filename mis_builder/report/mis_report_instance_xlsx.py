...
class MisBuilderXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        # document title
        bold = workbook.add_format({"bold": True,"align": "center","bg_color": "#F0EEEE"})
        bg_bold = workbook.add_format({"bold": True,"align": "center"})
        not_bold = workbook.add_format({"bold": False,"align": "left"})
        header_format = workbook.add_format(
            {"bold": True, "align": "center", "bg_color": "#F0EEEE"}
        )
        if objects[0].head_name == 'dept_wise':

            name = 'Income Statement - Dept Wise'
        elif objects[0].head_name == 'month_wise':
            name = 'Income Statement - Monthly'
        elif objects[0].head_name == 'summary':
            name = 'Income Statement - Summary'
        elif objects[0].head_name == 'comparitive':
            name = 'Income Statement - Comparative'


        if objects[0].is_balance_sheet and objects[0].compare_year:
            name = 'BALANCE SHEET - COMPARATIVE'
        elif objects[0].is_balance_sheet and not objects[0].compare_year:
            name = objects[0].report_id.description
        elif objects[0].is_project:
            name = objects[0].report_id.description
        if not objects[0].is_project:

            sheet.merge_range('A1:D1', 'GAMBIT GULF CONTRACTING CO. LTD.', bold)                      
            # sheet.write(row_pos, B1:B2, "GAMBIT GULF CONTRACTING CO. LTD.", bold)
            row_pos += 1
            sheet.merge_range('A2:D2', name, bold)
            # sheet.write(row_pos, 1:2, name, bold)
            row_pos += 1
        else:
            sheet.merge_range('A1:D1', 'GAMBIT GULF CONTRACTING CO. LTD.', bg_bold)                      
            # sheet.write(row_pos, B1:B2, "GAMBIT GULF CONTRACTING CO. LTD.", bold)
            row_pos += 1
            sheet.merge_range('A2:D2', name, bg_bold)
            # sheet.write(row_pos, 1:2, name, bold)
            row_pos += 1
        if not objects[0].is_project:
            sheet.merge_range('A3:D3', objects[0].note, bold)
            # sheet.write(row_pos, 1:2, objects[0].note, bold)
            row_pos += 2
        if objects[0].is_project:
            sheet.write(row_pos, 0, "Year", not_bold)
            sheet.write(row_pos, 1, objects[0].period_year, not_bold)
            row_pos += 1
            sheet.write(row_pos, 0, "Period", not_bold)
            sheet.write(row_pos, 1, objects[0].period_month, not_bold)
            row_pos += 1
            sheet.write(row_pos, 0, "Project Code", not_bold)
            sheet.write(row_pos, 1, objects[0].project_codes, not_bold)
            row_pos += 1
            if len(objects[0].project_ids) == 1:
                sheet.write(row_pos, 0, "Project Name", not_bold)
                sheet.write(row_pos, 1, objects[0].project_name, not_bold)
                row_pos += 1
                sheet.write(row_pos, 0, "Customer Name", not_bold)
                sheet.write(row_pos, 1, objects[0].customer_name, not_bold)
                row_pos += 1
        ...
        # column headers
        if objects[0].is_balance_sheet and objects[0].report_id.name != 'BALANCE SHEET - SUMMARY':
            sheet.write(row_pos, 0, "Account ID", header_format)
        
            sheet.write(row_pos, 1, "Description", header_format)
            col_pos = 2
        else:
            sheet.write(row_pos, 0, "Description", header_format)
            col_pos = 1
        # sheet.write(row_pos, col_pos, "Description", header_format)
        for col in matrix.iter_cols():
            label = col.label or "Description"
        ...
        for row in matrix.iter_rows():
            ...
            if row.description:
                ...
                sheet.set_row(row_pos, ROW_HEIGHT * 2)
            if objects[0].is_balance_sheet and objects[0].report_id.name != 'BALANCE SHEET - SUMMARY':
                sheet.write(row_pos, col_pos, row.account_code, row_format)            
                col_pos = col_pos + 1
        ...
        # adjust col widths
        if objects[0].is_balance_sheet and objects[0].report_id.name != 'BALANCE SHEET - SUMMARY':
            sheet.set_column('B:B', max(label_col_width, MAX_COL_WIDTH) * COL_WIDTH)
            sheet.set_column('A:A',15)
            data_col_width = min(MAX_COL_WIDTH, max(col_width.values()))
            min_col_pos = min(col_width.keys())
            max_col_pos = max(col_width.keys())
            sheet.set_column(min_col_pos+1, max_col_pos, data_col_width * COL_WIDTH)
        else:    

            sheet.set_column(0, 0, max(label_col_width, MAX_COL_WIDTH) * COL_WIDTH)
            data_col_width = min(MAX_COL_WIDTH, max(col_width.values()))
            min_col_pos = min(col_width.keys())
            max_col_pos = max(col_width.keys())
