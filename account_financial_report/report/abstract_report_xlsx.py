...
class AbstractReportXslx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        report_title = self._get_report_titler(objects, data=data)
        ...
        self._write_report_titler(report_title, report_data)
    ...
    def _define_formats(self, workbook, report_data):
        ...
        report_data["formats"]["format_general"].set_num_format(
            "#,##0" 
        )
    ...
    def _write_report_titler(self, title, report_data):
        """Write report title on current line using all defined columns width.
        Columns are defined with `_get_report_columns` method.
        """
        report_data["sheet"].merge_range(
            ...
            if cell_type == "string":
                if (
                    ...
                    if (
                        ...
                        value = value and value.strftime("%m/%d/%Y")
                    report_data["sheet"].write_string(
                        report_data["row_pos"], col_pos, value or ""
                    )
            elif cell_type == "amount":
                ...
                if (
                    ...
                    cell_format = report_data["formats"]["format_general"]
                report_data["sheet"].write_number(
                    report_data["row_pos"], col_pos, float(value), cell_format
                )
            elif cell_type == "amount_currency":
