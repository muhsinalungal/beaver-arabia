...
class GeneralLedgerXslx(models.AbstractModel):
    ...
    def _get_report_columns(self, report):
        ...
        if report.show_journal_code:
            res += [
                {
                    "header": _("Journal Code"),
                    "field": "journal_code",
                    "width": 20,
                },
            ]

        if report.show_employee:
            res += [
                {
                    "header": _("Employee"),
                    "field": "employee",
        ...
        if report.show_accommodation:
            res += [
                {
                    "header": _("Accommodation"),
                    "field": "accommodation",
                    "width": 20,
                },
            ]
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        ...
        for account in general_ledger:
            ...
            if not account["partners"]:
                ...
                if account["init_bal"]["balance"] < 0:
                    account.update(
                        {
                            # "initial_debit": account["init_bal"]["debit"],
                            "initial_credit": account["init_bal"]["debit"] - account["init_bal"]["credit"],
                    ...
                    account.update(
                        {
                            ...
                            "initial_debit": account["init_bal"]["debit"] - account["init_bal"]["credit"],
                            "initial_balance": account["init_bal"]["balance"],
                        }
                    )
    ...
    def write_initial_balance_from_dict(self, my_object, report_data):
        ...
        if "partner" in my_object["type"]:
            label = _("Partner Opening balance")
