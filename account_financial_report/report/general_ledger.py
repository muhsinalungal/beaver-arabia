...
class GeneralLedgerReport(models.AbstractModel):
    ...
    def _get_final_balances_domain(
    ...
    def _get_accounts_final_balance(self, final_balance):
        
        gl_final_acc_pl = self.env["account.move.line"].read_group(
            domain=final_balance,
            fields=["account_id", "debit", "credit", "balance", "amount_currency"],
            groupby=["account_id"],
        )
        gl_fianl_acc = gl_final_acc_pl
        return gl_fianl_acc
    ...
    def _get_initial_balance_data(
        ...
        asset_ids,
        accommodation_ids,
        account_cost_center_ids,
        account_journal_code_ids,
        cost_center_ids,
        extra_domain,
    ):
        ...
        if department_ids:
            base_domain += [("cost_center_id", "in", department_ids)]
        if employee_ids:
            base_domain += [("employee_id", "in", employee_ids)]
        if asset_ids:
            base_domain += [("asset_id", "in", asset_ids)]
        if accommodation_ids:
            base_domain += [("accomadation_id", "in", accommodation_ids)]
        if account_cost_center_ids:
            base_domain += [("budgetry_position_id", "in", account_cost_center_ids)]
        if account_journal_code_ids:
            base_domain += [("move_id.journal_code_id", "in", account_journal_code_ids)]
        if cost_center_ids:
            base_domain += ['|',("analytic_account_id", "in", cost_center_ids),("move_id.analytic_account_id", "in", cost_center_ids)]
        if extra_domain:
        ...
        final_domain = self._get_final_balances_domain(
        ...
        gl_final_acc = self._get_accounts_final_balance(
            final_domain
        ) 
        ...
        for gl in gl_initial_acc:
            ...
            gen_ld_data[acc_id]["fin_bal"]["credit"] = 0
            gen_ld_data[acc_id]["fin_bal"]["debit"] = 0
        ...
        if gl_initial_acc_prt:
            for gl in gl_initial_acc_prt:
                ...
                gen_ld_data[acc_id][prt_id]["fin_bal"]["credit"] = 0
                gen_ld_data[acc_id][prt_id]["fin_bal"]["debit"] = 0
    ...
    def _get_move_line_data(self, move_line):
        move_line_obj = self.env["account.move.line"].browse(move_line["id"])
        move_line_data = {
            "id": move_line["id"],
            "date": move_line["date"],
            "entry": move_line_obj.move_id.name,
            ...
            "cost_center": move_line_obj["cost_center_id"][0].name
            if move_line_obj["cost_center_id"]
            else "",
            "cost_center_id": move_line_obj["cost_center_id"][0].name
            if move_line_obj["cost_center_id"]
            else False,

            "journal_code": move_line_obj["journal_code_id"][0].name
            if move_line_obj["journal_code_id"]
            else "",
            "journal_code_id": move_line_obj["journal_code_id"][0].name
            if move_line_obj["journal_code_id"]
            else False,

            "employee": move_line_obj["employee_id"][0].name
            if move_line_obj["employee_id"]
            else "",
            "employee_id": move_line_obj["employee_id"][0].name
            if move_line_obj["employee_id"]
            else False,

            "asset": move_line_obj["asset_id"][0].name
            if move_line_obj["asset_id"]
            else "",
            "asset_id": move_line_obj["asset_id"][0].name
            if move_line_obj["asset_id"]
            else False,

            "accommodation": move_line_obj["accomadation_id"][0].name
            if move_line_obj["accomadation_id"]
            else "",
            "accomadation_id": move_line_obj["accomadation_id"][0].name
            if move_line_obj["accomadation_id"]
            else False,

            "cost": move_line_obj["budgetry_position_id"][0].name
            if move_line_obj["budgetry_position_id"]
            else "",
            "budgetry_position_id": move_line_obj["budgetry_position_id"][0].name
            if move_line_obj["budgetry_position_id"]
            else False,
        ...
        if move_line_obj["journal_code_id"]:
            move_line_data.update({"journal_code_id": move_line_obj["journal_code_id"]})

        if move_line_obj["cost_center_id"]:
            move_line_data.update({"cost_center_id": move_line_obj["cost_center_id"]})

        if move_line_obj["employee_id"]:
            move_line_data.update({"employee_id": move_line_obj["employee_id"]})

        if move_line_obj["asset_id"]:
            move_line_data.update({"asset_id": move_line_obj["asset_id"]})

        if move_line_obj["accomadation_id"]:
            move_line_data.update({"accomadation_id": move_line_obj["accomadation_id"]})

        if move_line_obj["budgetry_position_id"]:
            move_line_data.update({"budgetry_position_id": move_line_obj["budgetry_position_id"]})
        ...
        if move_line_obj.name == move_line_obj.move_id.name and move_line_obj.extra_payment_id:
           ref_label =  move_line_obj.extra_payment_id.ref
        elif move_line_obj.payment_id:
            ref_label =  move_line_obj.payment_id.ref
        # elif move_line_obj.payment_id and move_line_obj.payment_id.partner_type == 'supplier' and move_line_obj.credit >0.00:
        #     ref_label =  move_line_obj.payment_id.ref
        else:
           ref_label = move_line_obj.name 
        move_line_data.update({"ref_label": ref_label})
        if move_line_obj.analytic_account_id:
            move_line_data.update({"analytic_account": move_line_obj.analytic_account_id.name,
                                    "analytic_account_id" : move_line_obj.analytic_account_id.id})
        if not move_line_obj.analytic_account_id and move_line_obj.move_id.analytic_account_id:
            move_line_data.update({"analytic_account": move_line_obj.move_id.analytic_account_id.name,
                                    "analytic_account_id" : move_line_obj.move_id.analytic_account_id.id})
    ...
    def _get_period_domain(
        ...
        if department_ids:
            domain += [("cost_center_id", "in", department_ids)]
        if employee_ids:
            domain += [("employee_id", "in", employee_ids)]
        if asset_ids:
            domain += [("asset_id", "in", asset_ids)]
        if accommodation_ids:
            domain += [("accomadation_id", "in", accommodation_ids)]
        if account_cost_center_ids:
            domain += [("budgetry_position_id", "in", account_cost_center_ids)]
        if account_journal_code_ids:
            domain += [("move_id.journal_code_id", "in", account_journal_code_ids)]
        if cost_center_ids:
            domain += ['|',("analytic_account_id", "in", cost_center_ids),("move_id.analytic_account_id", "in", cost_center_ids)]
    ...
    def _get_report_values(self, docids, data):
        ...
        account_cost_center_ids = data["account_cost_center_ids"]
        account_journal_code_ids = data["account_journal_code_ids"]
