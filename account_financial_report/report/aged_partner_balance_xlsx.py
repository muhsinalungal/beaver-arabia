...
class AgedPartnerBalanceXslx(models.AbstractModel):
    ...
    def _get_report_name(self, report, data=False):
        ...
        if company_id:
            ...
            report_name = suffix
        return report_name
    def _get_report_titler(self, report, data=False):
        company_id = data.get("company_id", False)
        report_title = _("Aged Partner Balance")
        if company_id:
            company = self.env["res.company"].browse(company_id)
    ...
    def _get_report_columns(self, report):
        if not report.show_move_line_details:
            return {
                0: {"header": _("Partner ID"), "field": "code", "width": 14},
        ...
        return {
            ...
            0: {"header": _("Invoice No"), "field": "entry", "width": 18},
            1: {"header": _("Inv Type"), "field": "inv_type", "width": 8},
            2: {"header": _("JRL"), "field": "journal", "width": 8},
            3: {"header": _("JRL Code"), "field": "journal_code", "width": 8},
            # 3: {"header": _("Account"), "field": "account", "width": 9},
            # 4: {"header": _("Partner"), "field": "partner", "width": 25},
            4: {"header": _("Ref - Label"), "field": "ref_label", "width": 40},
            5: {"header": _("Date"), "field": "date", "width": 11},
            6: {"header": _("Due date"), "field": "due_date", "width": 11},
            7: {"header": _("Due Days"), "field": "due_days","type": "general", "width": 11},
            8: {
                "header": _(u"0-15 Days"),
                "field": "current",
                "field_footer_total": "current",
                "field_footer_percent": "percent_current",
    ...
    def _get_report_filters(self, report):
        return [
            [_("Date at filter"), report.date_at.strftime("%m/%d/%Y")],
            [
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        ...
        if not show_move_line_details:
            ...
            for account in aged_partner_balance:
                ...
                # For each partner
                sorted_partners = sorted(account["partners"], key=lambda partner: partner["code"])
                for partner in sorted_partners:
                    ...
                    if partner["code"]:
                        ...
                        code = ''
                    self.write_array_title(code + " - " +partner["name"], report_data)
